"use client"

import { ArrowLeft, ShieldCheck } from "lucide-react"
import { InkPillButton, OutlinedPillButton } from "@/components/ui/mastercard-design"
import { AccommodationDetails, TravelSubView } from "./types"

interface Props {
  accommodation?: AccommodationDetails
  onBack: () => void
  onNavigate: (view: TravelSubView) => void
}

export function AccommodationTravelSubView({ accommodation, onBack }: Props) {
  const isEngaged = accommodation?.isEngaged || false
  const statusLabel = isEngaged 
    ? (accommodation?.status === 'confirme' || accommodation?.status === 'traite' ? 'Réservé' : 'En cours de réservation')
    : 'Non engagé'

  const items = [
    {
      title: "Hôtel Partenaire ou Appart-Hôtel PMR",
      desc: "Chambre accessible PMR, ascenseur médical, climatisation et environnement calme",
      status: isEngaged ? (accommodation?.hotelName || "Sélectionné") : "Non engagé",
    },
    {
      title: "Proximité Immédiate Clinique",
      desc: "Localisation à moins de 10 à 15 minutes des plateaux techniques hospitaliers",
      status: isEngaged ? "Garanti" : "Non engagé",
    },
    {
      title: "Hébergement des Accompagnants",
      desc: "Formule lit d'appoint ou chambre double pour les proches voyageant avec le patient",
      status: isEngaged && accommodation?.accompanyingPersons ? `${accommodation.accompanyingPersons} accompagnant(s)` : "Sur demande",
    },
    {
      title: "Restauration Adaptée Post-Opératoire",
      desc: "Menus équilibrés conformes aux préconisations diététiques du corps soignant",
      status: "Sur demande",
    }
  ]

  return (
    <div className="space-y-8">
      {/* En-tête */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-[#E2DDD7]">
        <div className="flex items-center gap-4">
          <button
            onClick={onBack}
            className="w-10 h-10 rounded-full border border-[#E2DDD7] bg-[#FCFBFA] flex items-center justify-center hover:bg-[#141413] hover:text-white transition-colors shrink-0"
            title="Retour à la synthèse"
          >
            <ArrowLeft className="w-4 h-4" />
          </button>
          <div>
            <span className="text-[11px] font-mono font-bold text-[#CF4500] uppercase tracking-wider block">
              SÉJOUR & HÔTELLERIE
            </span>
            <h2 className="text-2xl sm:text-3xl font-medium text-[#141413] tracking-tight">
              Hébergements & Résidences Partenaires
            </h2>
          </div>
        </div>

        <span className={`text-xs font-mono font-bold px-4 py-2 rounded-full self-start sm:self-auto ${
          isEngaged ? "bg-[#141413] text-white" : "bg-[#F4F2EE] text-[#696969]"
        }`}>
          Statut : {statusLabel}
        </span>
      </div>

      {/* Cartes métriques */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-[#F4F2EE] border border-[#E2DDD7] rounded-[24px] p-5 flex flex-col justify-between">
          <span className="text-xs font-bold text-[#696969] uppercase tracking-wider">État Réservation</span>
          <span className="text-xl sm:text-2xl font-medium text-[#141413] tracking-tight mt-3">{statusLabel}</span>
          <span className="text-[11px] text-[#696969] mt-1">Séjour hôtelier</span>
        </div>

        <div className="bg-[#F4F2EE] border border-[#E2DDD7] rounded-[24px] p-5 flex flex-col justify-between">
          <span className="text-xs font-bold text-[#696969] uppercase tracking-wider">Résidence</span>
          <span className="text-xl sm:text-2xl font-medium text-[#141413] tracking-tight mt-3">
            {accommodation?.hotelName || "--"}
          </span>
          <span className="text-[11px] text-[#696969] mt-1">Établissement</span>
        </div>

        <div className="bg-[#F4F2EE] border border-[#E2DDD7] rounded-[24px] p-5 flex flex-col justify-between">
          <span className="text-xs font-bold text-[#696969] uppercase tracking-wider">Arrivée (Check-In)</span>
          <span className="text-xl sm:text-2xl font-medium text-[#141413] tracking-tight mt-3">
            {accommodation?.checkInDate || "--"}
          </span>
          <span className="text-[11px] text-[#696969] mt-1">Début de séjour</span>
        </div>

        <div className="bg-[#F4F2EE] border border-[#E2DDD7] rounded-[24px] p-5 flex flex-col justify-between">
          <span className="text-xs font-bold text-[#696969] uppercase tracking-wider">Départ (Check-Out)</span>
          <span className="text-xl sm:text-2xl font-medium text-[#141413] tracking-tight mt-3">
            {accommodation?.checkOutDate || "--"}
          </span>
          <span className="text-[11px] text-[#696969] mt-1">Fin de séjour</span>
        </div>
      </div>

      {/* Détail */}
      <div className="space-y-4 pt-2">
        <div className="flex items-center justify-between pb-3 border-b border-[#E2DDD7]">
          <h3 className="text-lg sm:text-xl font-medium text-[#141413] tracking-tight">
            Options d'Accueil et de Convalescence
          </h3>
          <span className="text-xs font-mono font-bold text-[#857F78]">
            4 Postes
          </span>
        </div>

        <div className="divide-y divide-[#E2DDD7]">
          {items.map((item, idx) => (
            <div key={idx} className="py-4 px-2 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div className="space-y-0.5 min-w-0 pr-4">
                <h4 className="text-sm font-semibold text-[#141413]">{item.title}</h4>
                <p className="text-xs text-[#696969]">{item.desc}</p>
              </div>

              <div className="flex items-center gap-4 self-end sm:self-center shrink-0">
                <span className="px-3 py-1 rounded-full text-[11px] font-mono font-medium uppercase bg-[#F4F2EE] text-[#857F78]">
                  {item.status}
                </span>
              </div>
            </div>
          ))}
        </div>
      </div>

      {/* Actions */}
      <div className="flex flex-wrap items-center gap-3 pt-4 border-t border-[#E2DDD7]">
        <InkPillButton href="/patient?view=messages">
          <span>Ajuster mon hébergement</span>
        </InkPillButton>
        <OutlinedPillButton onClick={onBack}>
          <span>Retour au récapitulatif voyage</span>
        </OutlinedPillButton>
      </div>

      {/* Note d'information */}
      <div className="p-5 rounded-[20px] bg-[#F4F2EE] border border-[#E2DDD7] flex items-start gap-3.5 text-xs text-[#696969]">
        <ShieldCheck className="w-5 h-5 text-[#CF4500] shrink-0 mt-0.5" />
        <p className="leading-relaxed">
          Toutes les chambres sélectionnées sont rigoureusement auditées pour le confort post-opératoire et adaptées aux personnes à mobilité réduite.
        </p>
      </div>
    </div>
  )
}
